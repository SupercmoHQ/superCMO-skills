...
Three endpoints on `meta_ad_library` matter here:

| Endpoint | What it's for |
| --- | --- |
| `company_ads` | one advertiser's ads — the main call |
| `search_companies` | find an advertiser's page id by name, when the name alone doesn't resolve |
| `search_ads` | ads across advertisers matching a keyword — used only when discovering candidate competitors |
...
**Never pull by `companyName`.** It is the cheap-looking route and it is how the wrong company's ads
end up in the analysis: the name matches whatever page the library thinks is closest — a retailer in
another country, a dormant page, a similarly-named business in an unrelated category — and the ads
come back looking perfectly legitimate. Short brand names collide constantly, and the same name under
a country filter can return nothing at all while the page id returns hundreds.
...
- **One candidate is obviously right** — verified, right country, far more followers than the rest, and
  a category that matches what the user described → take it, and say in one line which page you took.
...
ads only a few weeks old, and stop at the depth cap for this run or when the competitor runs out.
**Each page is another call** — don't pull everything a large advertiser has.
...
reach for it, and never read an empty date-filtered pull as evidence a competitor has no
long-running ads.

Where the depth cap is reached before any older ads appear, say in the analysis that the sample is
recency-weighted.

## What comes back

With `trim: true`, `social_research` projects an ad-library response down to the readable fields and
one media URL per ad. Each ad comes back with its run length, its campaign, its intent and its creative kind already
worked out, so none of that is counted by hand. `shaping` says what was dropped, and the whole
...
Repeat the pull with `status: "INACTIVE"`. These are the ads the competitor stopped, and they carry an
end date.
...
Alongside the ads, `advertisers` carries each page once — its name, follower count and self-declared
...
| Field | |
| --- | --- |
| `page_id`, `page_name` | the advertiser that ran it — the identity check above, and what sorts ads into competitors |
...
- **No page, or no ads at all** → record that this competitor isn't advertising in this market and
  move to the next one.
- **Fewer ads than the depth asked for** → analyse the ads there are. Every conclusion drawn from that
  competitor is labelled thin in the analysis, and no percentage is quoted for them.
- **Live ads but no stopped ones** → carry on. The analysis says the comparison against what they
  dropped couldn't be made for that competitor in this market.
...
- **An empty or whitespace param is dropped**, not defaulted. A call meant to be filtered goes out
  unfiltered.
- **Every call is billed, including one that returns nothing.** A misspelled competitor name costs the
  same as a good pull.

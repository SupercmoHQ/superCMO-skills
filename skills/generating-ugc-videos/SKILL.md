...
**How many clips.** One storyboard becomes one clip. `list_video_models` gives the clip model's
shortest and longest clip — read them there rather than assuming. Make each clip as long as the model
allows and let the leftover be the last one; where that leftover falls under the minimum, give the
last clip the minimum anyway and let the video run slightly over. On a 4–15s model: 30s is two clips
of 15, 50s is 15, 15, 15 and 5, and 18s is 15 and 4.

**What the story can hold.** A **primary action** is one continuous motion at one object — picking a
thing up, tilting it, putting it on, pointing at it. Each one needs two seconds where possible, three at most, so a clip's
...
Trigger the `writing-video-scripts` skill workflow with a brief of what the creator should
communicate, the concept from Step 5 — the clip durations, the beats and the word budget — spoken on
camera and lip-synced, and only the claims the user supplied, used as written. None supplied means
none made.
...
Step 4 product description carried in unchanged, and the look — a phone photograph in available light, sensor grain and real skin, no studio lighting. How the clip divides into panels is worked out
...
the video production, and ask them to check the product in particular — that it is rendered right in every panel
it appears in. They can change any panel: rebuild only the sheet that panel is on, show it, and keep
going until they approve. Where the user has said they don't want to be asked, say in one line what
you're going with and carry on.
...
of that sheet, the settled clip model with audio on at that clip's duration from the concept and the delivery
...
- Per request: the `prompt` from Step 10 · `model` the settled clip model · `duration` for that clip
  from the concept · `generate_audio` on.
...
- **A clip is refused because a reference image may show a real person** → not the wardrobe and not
  ...
  recasting a third time — the one thing that reopens the settled clip model mid-job.

...
or restyling an existing product photo — hand off to the **`product-photoshoot`** skill, which owns
that surface. Stay here for everything else: general images, graphics and posters, portraits,
...
Share the resulting image URL(s) (and local file paths) with the user.
...
- **Generic failure** → resubmit as-is once → simplify the prompt → switch to a fallback model.

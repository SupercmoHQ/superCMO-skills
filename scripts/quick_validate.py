...
def read_frontmatter(file_path):
    """Return (raw_frontmatter_text, parsed_flat_dict) or (None, None)."""
    try:
        ...
        for line in raw.splitlines():
            ...
            if not s or s.startswith("#"):
                continue
            if ":" in line and not line.startswith(" "):  # top-level keys only
...
def lint_frontmatter(raw, fm, folder):
    ...
    for line in raw.splitlines():
        ...
        if re.match(r"^\s*hermes\s*:", line):
            errors.append("forbidden nested 'metadata.hermes' — use flat metadata")

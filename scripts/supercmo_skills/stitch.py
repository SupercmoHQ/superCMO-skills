...
def _trim(ffmpeg, path, t_in, t_out, workdir, i):
    ...
    if not math.isfinite(start) or (stop is not None and not math.isfinite(stop)):
        return None, f"in/out must be finite seconds; got in={t_in!r}, out={t_out!r}"
    if start < 0 or (stop is not None and stop <= start):
        return None, f"trim needs 0 <= in < out (seconds); got in={start}, out={stop}"
    dur = _dur(path)
    if dur:
        if start >= dur:
            return None, f"in={start} is at/after the clip's end ({dur:.2f}s)"
        if stop is not None and stop > dur + 0.05:
            return None, f"out={stop} is beyond the clip's end ({dur:.2f}s)"
    cmd = [ffmpeg, "-y"]
    if start:
        cmd += ["-ss", str(start)]
    cmd += ["-i", path]

...
def _num(value, fallback=0):
    """Coerce a caller-supplied number; junk falls back (same philosophy as _ass.colour)."""
    try:
...
def _style(video_h, overrides):
    o = overrides or {}
    fsize = _num(o.get("font_size")) or max(16, round(video_h * 0.06))
    return {
        ...
        "words_per_line": max(1, _num(o.get("words_per_line"), 5)),
        "karaoke": o.get("karaoke", True),
        "bold": o.get("bold", True),
        "outline": _num(o.get("outline")) or max(2, round(fsize * 0.08)),
        "margin_v": _num(o.get("margin_v")) or round(video_h * 0.10),
    }

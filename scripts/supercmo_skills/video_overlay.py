...
def _texts_ass(texts, size):
    ...
    for t in texts or []:
        ...
        try:
            fs = int(float(t.get("font_size") or 0)) or max(20, round(h * 0.05))
        except (TypeError, ValueError):
            fs = max(20, round(h * 0.05))
...
def _decorate(ffmpeg, src, workdir, logo_path, logo_position, logo_scale, texts_ass, iw, match, out):
    ...
    if logo_path:
        ...
        steps.append(f"[1:v]scale={lw}:-2[lg]")   # -2 = auto height rounded to even (yuv420p-safe)
...
def video_overlay(video, logo=None, logo_position="bottom-right", logo_scale=0.15, texts=None,
    ...
    try:
        ...
        if texts and not texts_ass:
            return {"ok": False, "error": "texts had no usable entry — each needs `text` plus numeric "
                    "`start` and `end` in seconds.", "hint": "fix the texts list or drop the parameter"}
        if texts_ass:
        ...
        if not end_card:
            if decorated:
                shutil.move(stage1, out)
            else:
                rc, err = _ffmpeg.run([ffmpeg, "-y", "-i", src, "-c", "copy", out])
                if rc != 0 or not _ffmpeg.ok(out):
                    return {"ok": False, "error": "ffmpeg could not write the output.",
                            "hint": "confirm the video is a valid file", "detail": err[-500:]}
